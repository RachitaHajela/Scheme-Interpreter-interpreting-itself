...
(define (my-apply fn L)
		(cond ((eq? (car fn) 'primitive-function) (apply (car (cdr fn)) L))
			  ((eq? (car fn) 'closure) (handle-call (cons fn L)))
...
(define (top-eval exp)
  (cond ((not (pair? exp)) (my-eval exp *global-env*))
	((eq? (car exp) 'define)   
	 ( if (pair? (cadr exp)) ( top-eval (list 'define (car (cadr exp)) (cons 'lambda (cons (cdr (cadr exp )) (cddr exp))) ) )
	 (insert! (list (cadr exp) (my-eval (caddr exp) *global-env*)) *global-env*))
	 ) ; just return the symbol being defined
...
( let ((new-env (append (create-assolist vars env) env )))
   (handle-block body new-env))
)

(define (create-assolist vars env)
(cond (( null? vars) '())
       (else ( cons (list (caar vars) (my-eval  (cadar vars) env )) ( create-assolist (cdr vars) env)) 
)))
...
(define (create-assolist* vars env)
(cond (( null? vars) env)
       (else (let ((new-env ( cons (list (caar vars) (my-eval  (cadar vars) env )) env))) 
		          (create-assolist* (cdr vars) new-env))
)))

(define	(handle-letrec vars body env)
( let ((new-env (append (create-assolistrec vars env) env )))
      ( modify-env vars new-env)
      ( handle-block body new-env)))
   
(define (modify-env vars env)
	(cond ((null? vars))
		  ( else (set-cdr! (assoc (caar vars) env) (list (my-eval (cadar vars) env)))
		          (modify-env (cdr vars) env))
		   ))

(define (create-assolistrec vars env)
(cond (( null? vars) '())
       (else ( cons (list (caar vars) '*uninitialized*) ( create-assolistrec (cdr vars) env)) 
)))
